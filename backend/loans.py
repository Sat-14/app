...
@loans_bp.route('/loans/<bill_id>', methods=['POST'])
@jwt_required()
def create_loan(bill_id):
    """
    Creates a new loan entry linked to a specific bill.
    This endpoint should be called after a bill with category 'loan' is created.
    """
    user_id = get_jwt_identity()
    logger.info(f"[LOANS CREATE] Request to create loan for bill_id: {bill_id} by user: {user_id}")
    
    bill = Bill.query.filter_by(id=bill_id, user_id=user_id).first()
    if not bill:
        logger.warning(f"[LOANS CREATE] Bill {bill_id} not found for user {user_id}")
        return jsonify({'message': 'Bill not found'}), 404

    data = request.get_json()
    if not data:
        logger.warning(f"[LOANS CREATE] No data provided for loan creation.")
        return jsonify({'message': 'Loan details required'}), 400
    ...
    try:
        loan = LoanDetails(
            ...
            interest_rate_percent=data.get('interest_rate_percent', 0)
        )
        
        db.session.add(loan)
        db.session.commit()
        logger.info(f"[LOANS CREATE] Successfully created loan for bill {bill_id}")
        ...
        db.session.rollback()
        logger.error(f"[LOANS CREATE ERROR] A loan already exists for bill {bill_id}")
        return jsonify({'message': 'A loan entry already exists for this bill'}), 409
    except Exception as e:
        db.session.rollback()
        logger.error(f"[LOANS CREATE ERROR] Failed to create loan for bill {bill_id}: {str(e)}", exc_info=True)
        return jsonify({'message': 'Failed to create loan details'}), 500
