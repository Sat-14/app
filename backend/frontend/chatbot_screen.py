...
class ChatMessage(BoxLayout):
    ...
    color=(1, 1, 1, 1),
        ...
        msg_box = BoxLayout(
    size_hint_y=None,
    padding=dp(15),  # Increase padding
    height=msg_label.height + dp(30)  # More space
)
        ...
        self.height = msg_container.height
...
class ChatbotScreen(Screen):
    ...
    def show_welcome_message(self):
     def add_welcome_delayed(dt):
        ...
        # Force scroll to bottom with additional delay
        def fix_scroll(dt2):
    ...
    def load_suggestions(self):
        ...
        def get_suggestions():
            try:
                ...
                if response and response.status_code == 200:
                    data = response.json()
                    Clock.schedule_once(lambda dt: self.display_suggestions(data['suggestions']), 0)
            except Exception as e:
    ...
    def display_suggestions(self, suggestions):
        ...
        for suggestion in suggestions[:4]:
            ...
            btn.bind(on_release=lambda x, s=suggestion: self.send_suggestion(s))
